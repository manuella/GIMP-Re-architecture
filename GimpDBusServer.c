/*
 * GimpDBusServer- A program that publishes the GIMP 
...
query (void)
{
  ...
  gimp_install_procedure (
			  ...
			  "Copyright GIMPDBUS 2012",
			  "2012",
			  "<Toolbox>/Xtns/MediaScript/GimpDbusServer",
			  NULL, // was "RGB*, GRAY*",
			  GIMP_PLUGIN,
			  G_N_ELEMENTS (args), 0,
			  args, NULL);
...
/* gimpbus_g_variant_to_gimp_param- converts a single GVariant to a 
   GimpParam, which gets stored in the already allocated param.  
  ...
  for (i = 0; i < nvalues; i++)
    {
      ...
      if(values[i].type == GIMP_PDB_STRINGARRAY || GIMP_PDB_INT8ARRAY || 
	 GIMP_PDB_INT32ARRAY || GIMP_PDB_INT16ARRAY || GIMP_PDB_FLOATARRAY)
...
/* procnamesbuilder - makes a gimpnames struct with all 
...
methodmaker (struct gimpnames *nms)
{
  ...
  /*
    error: at procedure 309 (maybe 310) generate_pdb_method_info fails 
    and causes the plugin to crash, specifically I think it crashes in 
  */
     ...
     GimpParam       **return_vals)
{
  ...
  *return_vals  = values;
  ...
  owner_id = g_bus_own_name (G_BUS_TYPE_SESSION,
                             "edu.grinnell.cs.glimmer.GimpDbusServertest",
  ...
  fprintf (stderr, "GimpDbusServer process %d is pausing ...\n", pid);
